...
import { addAddress } from "../controllers/addressController_user.js";
...
export const AddressModel = {
  ...
  addAddress: async (tenantPool, data) => {
    ...
    const res =
      await tenantPool.query(`INSERT INTO tbl_address(user_id,address_type,name,phone,
      pincode,
      state,
      district,
      city,
 ...
 lng,
  ...
  listAddresses: async (tenantPool, user_id) => {
    const res = await tenantPool.query(
      `SELECT * FROM tbl_address WHERE user_id=$1 ORDER BY is_default DESC, created_at DESC`,
      [user_id]
    );
    return res.rows;
  },
